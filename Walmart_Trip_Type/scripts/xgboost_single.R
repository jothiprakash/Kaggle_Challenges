setwd("D:/Git/Useful Repos/Walmart_TripType_Classification_raj2014")
...
train_matrix<-read.csv(file="featureVectors/train_matrix.csv",
                       header=TRUE)
trainCluster<-read.csv(file="featureVectors/trainCluster.csv",
                       header=TRUE)
deptrainCluster<-read.csv(file="featureVectors/deptrainCluster.csv",
                          header=TRUE)


#joining the cluster features from department and fineline fields
train_matrix<-merge(train_matrix,
                    trainCluster,
                    by="VisitNumber")
train_matrix<-merge(train_matrix,
                    deptrainCluster,
...
# How to fill y properly : Xgboost requirement ,label must
# be in [0, num_class)
...
bst = xgboost(params=param,
              data = as.matrix(train_matrix[,-c(1,2)]),
              label = target,
              nrounds=440)
...
test_matrix <- read.csv(file = "featureVectors/test_matrix.csv",
                        header = TRUE)
testCluster <- read.csv(file = "featureVectors/testCluster.csv",
                        header = TRUE)
deptestCluster <- read.csv(file = "featureVectors/deptestCluster.csv",
                           header = TRUE)

test_matrix <- merge(test_matrix,
                     testCluster,
                     by = "VisitNumber")
test_matrix <- merge(test_matrix,
                     deptestCluster,
