...
# origin dataset with unique visitnumber and triptype pair
dataset = data.frame(unique(trainbase[, c(1, 2)]))    
head(dataset)
# creates dummy variable
traindmy = dummyVars(" ~ .", data = trainbase, fullRank = FALSE) 
str(traindmy)
# "flatten" the factor variables
traindmy2 = data.frame(predict(traindmy, newdata = trainbase))
head(traindmy2)

############### feature creation for department description 
# and day of the week. changing it to dummy

# creating new features for DepartmentDescription (DD)
DD_names = names(traindmy2)[c(3:9, 12:80)] 
# all zeros for DD first
DD_names2 = matrix(rep(0, 76 * 95674), 95674, 76) 
# combine with origin empty datasets
dataset = data.frame(dataset, DD_names2) 
# giving correct names for the DD
names(dataset)[3:78] =  DD_names    
for (i in 1:dim(dataset)[1]) {
  hitlist = traindmy2[traindmy2$VisitNumber == dataset[i, 2],]
  # taking the day of week out
  dayofweek = names(hitlist)[3:9][hitlist[1, 3:9] > 0]       
  # populating the day of week columns
  dataset[i, dayofweek] = 1      
  
  hitlist2 = trainbase[trainbase$VisitNumber == dataset[i, 2], ]
  agg = aggregate(ScanCount ~ DepartmentDescription, 
                  data = hitlist2, sum)
...
# origin dataset with unique visitnumber and triptype pair
testdataset = data.frame(unique(testbase[, c(1)]))    
...
# creates dummy variable
testdmy2 = dummyVars(" ~ .", data = testbase, fullRank = FALSE) 
# "flatten" the factor variables
testdmy3 = data.frame(predict(testdmy2, newdata = testbase))

for (i in 1:dim(testdataset)[1]) {
  hitlist = testdmy3[testdmy3$VisitNumber == testdataset[i, 1],]
  # taking the day of week out
  dayofweek = names(hitlist)[2:8][hitlist[1, 2:8] > 0]       
  # populating the day of week columns
  testdataset[i, dayofweek] = 1      
  
  hitlist2 = testbase[testbase$VisitNumber == testdataset[i, 1], ]
  agg = aggregate(ScanCount ~ DepartmentDescription, 
                  data = hitlist2, 
                  sum)
  sub1 = gsub(" ", ".", paste0(names(agg)[1], ".", agg[, 1]))
...
trainReturn = data.frame("gotReturn" = 1, 
                         "VisitNumber" = trainReturn)
head(trainReturn)
testReturn = data.frame("gotReturn" = 1, 
                        "VisitNumber" = testReturn)

traindata2 = merge(traindata, 
                   trainReturn, 
                   by = "VisitNumber", 
                   all.x = TRUE)
testdata2 = merge(testdata, 
                  testReturn, 
                  by = "VisitNumber", 
                  all.x = TRUE)
...
mapping = data.frame("TripType" = names(table(traindata2$TripType)), 
                     "mapped" = 0:37)
for (i in 1:dim(traindata2)[1]) {
  traindata2$TripType[i] = mapping[mapping$TripType == traindata2$TripType[i], 2]
}
...
for (i in 1:647054) {
  traindataset[unique_vis == train2[i, 1], 
               fln2 == train2[i, 3]] = traindataset[unique_vis == train2[i, 1], 
                                                    fln2 == train2[i, 3]] + train2[i, 2]
}

traindataset = data.frame("VisitNumber" = unique_vis, 
                          traindataset)
traindata3 = merge(traindata2, 
                   flnmat, 
                   by = "VisitNumber", 
                   all.x = TRUE)
sum(is.na(traindata3[, 1224]))
write.csv(traindataset, "traindataset_v2.csv", 
          row.names = FALSE)